...
test("Snake uses four shared leaderboard panels in two fixed columns", async () => {
  ...
  assert.ok(localBest, "Snake should render one local best entry per board size");
  ...
  assert.match(localBest[1], /gameStatsLocalState\.leaderboards\.snake\[size\]\[0\]/);
  assert.match(localBest[1], /label: "Best Local"/);
  assert.match(localBest[1], /hasEntry \? "#1" : "#—"/);
  assert.match(localBest[1], /Best local score:.*points/);
  assert.doesNotMatch(localBest[1], /playerRanks\.snake/);
  ...
  assert.match(renderSnake[1], /appendSnakeLocalBest\(leaderboard\.panel, size\)/);
